...
class VhalPropertyRelationshipAnalyzer:
    ...
    def analyze_property_relationships(
            cls, property_input: str) -> PropertyEcosystem:
                    ...
                    if rel_type == "dependent_on":
                        ...
                        relationships.append(
                            PropertyRelationship(
                                ...
                                description=cls._get_relationship_description(
                                    rel_type,
                                    prop),
                                priority=cls._get_relationship_priority(rel_type)))

        implementation_steps = cls.IMPLEMENTATION_PATTERNS.get(
            ecosystem_key, [])
    ...
    def _generate_usage_notes(ecosystem_key: str, config: Dict) -> List[str]:
        ...
        if "SEAT" in ecosystem_key:
            ...
            if "MEMORY" in ecosystem_key:
                notes.append(
                    "Memory systems should store all adjustable position properties")
            if "BELT" in ecosystem_key:
                notes.append(
                    "Safety critical: Belt properties may have regulatory requirements")
            ...
            if "HEATED" in ecosystem_key:
                notes.append(
                    "Heated surface properties may have safety timeouts")

        if config.get("safety_critical"):
            notes.append(
                "Safety Critical: This ecosystem includes safety-critical properties")
