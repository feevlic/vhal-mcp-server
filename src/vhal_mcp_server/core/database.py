...
class VhalPropertyDatabase:
    ...
    def search_properties(cls, keyword: str) -> List[VhalProperty]:
        ...
        for category in cls.PROPERTIES:
            if keyword_upper in category.value:
                ...
                matched_names.update(
                    prop.name for prop in cls._category_index[category])
        ...
        if not matches:
            for category, properties in cls.PROPERTIES.items():
                for name, prop_id in properties.items():
                    if (keyword_upper in name or keyword_lower in name.lower()
                            or any(term in name.lower() for term in search_terms)):
                        if name not in matched_names:
                            matches.append(VhalProperty(
                                name, prop_id, category))
                            matched_names.add(name)

        # Sort by relevance: exact > starts_with > contains
        exact_matches = [m for m in matches if keyword_upper == m.name]
        starts_with_matches = [m for m in matches if m.name.startswith(
            keyword_upper) and keyword_upper != m.name]
        contains_matches = [
            m for m in matches if keyword_upper in m.name and not m.name.startswith(keyword_upper)]
    ...
    def get_properties_by_category(
            cls, category: VhalCategory) -> List[VhalProperty]:
    ...
    def get_property_by_name(cls, name: str) -> VhalProperty:
        ...
        for category, properties in cls.PROPERTIES.items():
            if name_upper in properties:
                return VhalProperty(
                    name_upper, properties[name_upper], category)
        return None
...
class AndroidSourceLookup:
    ...
    def generate_search_url(keyword: str) -> str:
        ...
        return f"https://cs.android.com/search?q={
            keyword}%20automotive%20vehicle"
